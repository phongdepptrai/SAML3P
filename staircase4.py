...
def write_fancy_table_to_csv(ins, n, m, c, var_count, cons_count, peak, sol_count, solbb_count, status, time_taken, filename="staircase4_results.csv"):
...
def optimal(X,S,A,n,m,c,sol,solbb,start_time):
    ...
    while True:
        ...
        model = solve_with_timeout(solver, min(int(remaining_time), 60))  # Max 60s per iteration
...
def main():
    ...
    # Write CSV header
    with open("Output/staircase4_results.csv", "w", newline='') as f:
        writer = csv.writer(f)
