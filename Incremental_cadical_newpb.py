...
def optimal(X,S,A,n,m,c,sol,solbb,start_time):
    ...
    clauses , soft_clauses, var, U, solver1 = generate_new_pb(n,m,c, X, S, A, W, bestValue, lowval, clauses, start_var, solver)
...
def generate_new_pb(n,m,c, X, S, A, W, UB, LB, clauses, var_counter, solver):
    pos = [0]* n
    current_weight = 0
    for i in range(n):
        current_weight += W[i]
        pos[i] = min(current_weight, UB)
    true_value = var_counter + 1
    var_counter += 1
    clauses.append([true_value])
    U = [[[0 for t in range(c)] for j in range(pos[i] + 1)] for i in range(n)]
    for i in range(n):
    ...
    for t in range(c):
        ...
        for i in range(n):
            for j in range(1, W[i] + 1):
                clauses.append([-A[i][t], U[i][j][t]])
                solver.add_clause([-A[i][t], U[i][j][t]])
        
        # #pb2: R_{i-1, j} -> R_{i, j}
        for i in range(1, n):
            for j in range(1,pos[i-1] + 1):
                clauses.append([-U[i-1][j][t], U[i][j][t]])
                solver.add_clause([-U[i-1][j][t], U[i][j][t]])

        # #pb3: A_i & U_{i-1, j} -> U_{i, j + w_i}
        for i in range(1, n):
            for j in range(1,pos[i-1] + 1):
                if j + W[i] <= pos[i]:
                    clauses.append([-A[i][t], -U[i-1][j][t], U[i][j + W[i]][t]])
                    solver.add_clause([-A[i][t], -U[i-1][j][t], U[i][j + W[i]][t]])
        
        # #pb4: A_i -> ~U_{i-1, UB + 1 - w_i}
        for i in range(1, n):
            if UB + 1 - W[i] <= pos[i-1]:
                clauses.append([-A[i][t], -U[i-1][UB + 1 - W[i]][t]])
                solver.add_clause([-A[i][t], -U[i-1][UB + 1 - W[i]][t]])
    
    print("new pb constraints:", len(clauses))
    return clauses, [], var_counter, U[n - 1], solver
    


def write_fancy_table_to_csv(ins, n, m, c, val, s_cons, h_cons, peak, status, time_elapsed, filename="new_pb.csv"):
    global best_result
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ['-h', '--help', 'help']:
        ...
        print("")
        print("Files created:")
        print("  Output/new_pb.csv   # CSV results")
    ...
    if len(sys.argv) == 1:
        ...
        # Here
        for instance_id in range(25, len(file_name1)):
            ...
            command = f"wsl ./runlim -r {TIMEOUT} /mnt/c/Users/admin/Documents/Python/SAML3P/.venv/Scripts/python.exe incremental_cadical_newpb.py {instance_id}"
