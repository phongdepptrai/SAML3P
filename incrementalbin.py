...
def optimal(X,S,A,n,m,c,sol,solbb,start_time):
    ...
    clauses , soft_clauses, var, binU, solver1 = generate_binary_encoding(n,m,c, X, S, A, W, bestValue, lowval, clauses, start_var, solver)
    # Using incremental SAT solving with binary search
    current_best = bestValue
    while (True):
        if( time.time() - start_time >= 7200):
            print("Time limit exceeded.")
            return current_best, var, clauses, soft_clauses, "Time Limit Exceeded"
        model = solve(solver1)
        if model is None:
            print("No solution found maxsat.")
            return current_best, var, clauses, soft_clauses, "Optimal"
        ansmap, newValue = get_value2(n, m, c, model, W)
        print("best value:", newValue, end="\r")
        if newValue < current_best:
            current_best = newValue
            # Add constraint to exclude this value and search for better
            add_binary_constraint(solver1, binU, newValue - 1)
        else:
...
def generate_binary_encoding(n,m,c, X, S, A, W, UB, LB, clauses, var_counter, solver):
    """
    Generate binary encoding for peak power constraint.
    U[j] represents there is a possible peak <= j
    Uses binary variables to represent the peak value range [LB, UB]
    """
    soft_clauses = []
    
    # Calculate number of bits needed to represent values in range [LB, UB]
    n_bit = int(math.log2(UB - LB)) + 1 if UB > LB else 1
    
    # Create binary variables binU[0], binU[1], ..., binU[n_bit-1]
    # These represent the binary encoding of (peak - LB)
    binU = []
    for i in range(n_bit):
        binU.append(var_counter + 1)
        var_counter += 1
        # Add soft clause to minimize the peak value
        # Weight is 2^i to reflect the binary significance
        soft_clauses.append([[-var_counter], 2**i])
    
    var = var_counter + 1
    
    # For each time slot, ensure the power consumption doesn't exceed the binary-encoded peak
    for t in range(c):
        variables = []
        weights = []
        
        # Add power consumption variables for this time slot
        for i in range(n):
            variables.append(A[i][t])
            weights.append(W[i])
        
        # Add binary encoding variables (negated to represent upper bound)
        for i in range(n_bit):
            variables.append(-binU[i])
            weights.append(2**i)
        
        # Constraint: sum(W[i] * A[i][t]) <= LB + sum(2^i * binU[i])
        # Rearranged: sum(W[i] * A[i][t]) - sum(2^i * binU[i]) <= LB
        pb_clauses = PBEnc.leq(lits=variables, weights=weights, bound=LB, top_id=var)
    ...
    return clauses, soft_clauses, var, binU, solver

def add_binary_constraint(solver, binU, max_value):
    """
    Add constraint to exclude peak values >= max_value
    This is done by adding a constraint that the binary encoding represents a value < max_value
    """
    n_bit = len(binU)
    
    # We need to add constraint: binary_value < max_value
    # This is equivalent to: NOT(binary_value >= max_value)
    
    # Convert max_value to binary and create constraint
    max_binary = []
    temp_val = max_value
    for i in range(n_bit):
        max_binary.append(temp_val % 2)
        temp_val //= 2
    
    # Add constraint to ensure binary value < max_value
    # Find the most significant bit where we can force a difference
    for i in range(n_bit-1, -1, -1):
        if max_binary[i] == 1:
            # Create clause that forces this bit to be 0 OR some lower bit to be different
            clause = [-binU[i]]  # Force this bit to be 0
            # Add all ways that lower bits can make the value smaller
            for j in range(i):
                if max_binary[j] == 1:
                    clause.append(-binU[j])  # Any of these bits being 0 makes it smaller
            solver.add_clause(clause)
            break

def generate_inagural(n,m,c, X, S, A, W, UB, LB, clauses, var_counter, solver):
    """
    Original inaugural encoding function - kept for reference but not used
    """
    soft_clauses = []
    U = []
