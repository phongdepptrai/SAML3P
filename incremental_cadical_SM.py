...
def append_best_peak_snapshot_csv(result=None, filename="incremental_SM_best_peak_snapshot.csv"):
    global best_result
...
def upsert_result_csv(result, filename="Output/incremental_SM.csv"):
...
def write_fancy_table_to_csv(ins, n, m, c, val, s_cons, h_cons, peak, status, time_elapsed, filename="incremental_SM.csv"):
    global best_result
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        csv_file = 'Output/incremental_SM.csv'
        ...
        for instance_id in range(0, len(file_name1)):
            ...
            command = f"./runlim -r {TIMEOUT} {sys.executable} incremental_cadical_SM.py {instance_id}"
